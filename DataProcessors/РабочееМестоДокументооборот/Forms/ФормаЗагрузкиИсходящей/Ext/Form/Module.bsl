...
Процедура ПрочитатьНаСервере()
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	НазванияДокументов.Ссылка КАК СсылкаНазвания,
	               ...
	               |	НазванияДокументов.НазваниеДокумента + "" "" + НазванияДокументов.РасширениеНазвания КАК НазваниеДокумента00
	               |ИЗ
	               |	Справочник.НазванияДокументов КАК НазванияДокументов
	               |ГДЕ
	               |	НазванияДокументов.ТипДокумента = &ТипДокумента";
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ТипДокумента", Перечисления.ТипДокумента.исх);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтруктураНазвания = Новый Структура("Стадия, СсылкаНазвания", Выборка.Стадия, Выборка.СсылкаНазвания);
		СоответствиеСтадий.Вставить(НРег(Выборка.НазваниеДокумента), СтруктураНазвания);
	КонецЦикла;
	...
	Для НомерСтроки = 2 По ТаблицаВрем.Количество() - 1 Цикл
		...
		Для Каждого ЗначениеСоответсвия Из СоответсвиеКолонок Цикл
			...
			Если НРег(ИмяКолонки) = Нрег("Фио") Тогда
				СтруктураЗаполнения.Вставить("ДолжникСтрокой", ТекущееЗначение);
				
				Должник = Справочники.Должники.НайтиПоНаименованию(ТекущееЗначение);
			...
			ИначеЕсли НРег(ИмяКолонки) = Нрег("НомерКД") Тогда
				СтруктураЗаполнения.Вставить("НомерДоговора", ТекущееЗначение);
				Договор = Справочники.Договоры.НайтиПоНаименованию(ТекущееЗначение);
			...
			ИначеЕсли НРег(ИмяКолонки) = Нрег("Вид документа") Тогда
				СтруктураНазвания = СоответствиеСтадий.Получить(НРег(ТекущееЗначение));
				Если СтруктураНазвания = Неопределено Тогда
				...
				Иначе
					Стадия = СтруктураНазвания.Стадия;
					НазваниеДокументаСправочник = СтруктураНазвания.СсылкаНазвания;
				КонецЕсли;
				СтруктураЗаполнения.Вставить("НазваниеДокументаСправочник", НазваниеДокументаСправочник);
				СтруктураЗаполнения.Вставить("НазваниеДокумента", ТекущееЗначение);
			...
			ИначеЕсли НРег(ИмяКолонки) = Нрег("Сотрудник") Тогда
				Исполнитель = Справочники.Сотрудники.НайтиПоНаименованию(ТекущееЗначение);
