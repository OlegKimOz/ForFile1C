...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	//Вставить содержимое обработчика
	 Если Параметры.Свойство("тДоговор") Тогда
		   ...
		   Объект.ПервоначальныйКредитор=стрДоговор.Банк.Наименование;
		   ...
		   Объект.НомерИсполнительногоПроизводства=Параметры.НомерДела;
		   ...
		   КонецЕсли;
		        	...
		        	//{{КОНСТРУКТОР_ЗАПРОСА_С_ОБРАБОТКОЙ_РЕЗУЛЬТАТА
						...
						Запрос.Текст = 
							"ВЫБРАТЬ
							|	ВкладкаИсполнительноеПроизводство.Ссылка КАК Ссылка,
							|	ВкладкаИсполнительноеПроизводство.ЗаменаСтороныОтказано КАК ЗаменаСтороныОтказано,
							|	ВкладкаИсполнительноеПроизводство.ЗаменаСтороныУдовлетварено КАК ЗаменаСтороныУдовлетварено,
							|	ВкладкаИсполнительноеПроизводство.НомерИсполнительногоПроизводства КАК НомерИсполнительногоПроизводства,
							|	ВкладкаИсполнительноеПроизводство.ДатаВозбуждения КАК ДатаВозбуждения,
							|	ВкладкаИсполнительноеПроизводство.ДатаОкончания КАК ДатаОкончания,
							|	ВкладкаИсполнительноеПроизводство.Статья КАК Статья,
							|	ВкладкаИсполнительноеПроизводство.ОтделФССП КАК ОтделФССП,
							|	ВкладкаИсполнительноеПроизводство.Пристав КАК Пристав,
							|	ВкладкаИсполнительноеПроизводство.Телефон КАК Телефон
							|ИЗ
							|	Справочник.ВкладкаИсполнительноеПроизводство КАК ВкладкаИсполнительноеПроизводство
							|ГДЕ
							|	ВкладкаИсполнительноеПроизводство.Владелец = &Владелец";
						...
						ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
							Если ВыборкаДетальныеЗаписи.Количество()>0 Тогда
							     Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
									 ...
									 ссылкаВкладкаИсполнительноеПроизводство=ВыборкаДетальныеЗаписи.Ссылка;
									 Объект.ЗаменаСтороныУдовлетварено=ВыборкаДетальныеЗаписи.ЗаменаСтороныУдовлетварено;
									 Объект.ЗаменаСтороныОтказано=ВыборкаДетальныеЗаписи.ЗаменаСтороныОтказано;
									 Объект.ДатаВозбуждения=ВыборкаДетальныеЗаписи.ДатаВозбуждения;
									 Объект.ДатаОкончания=ВыборкаДетальныеЗаписи.ДатаОкончания;
									 Объект.Статья=ВыборкаДетальныеЗаписи.Статья;
									 Объект.ОтделФССП=ВыборкаДетальныеЗаписи.ОтделФССП;
									 Объект.Пристав=ВыборкаДетальныеЗаписи.Пристав;
									 Объект.ТелефонПристав=ВыборкаДетальныеЗаписи.Телефон;
					...
					ОтделФССПстрока= Объект.ОтделФССП.НазваниеОСП;	
			   ...
			   // найду дату суда если есть
				...
				//			|ГДЕ
				//			|	ВкладкаСудСудебнаяСтадияТЧ.Ссылка = &Ссылка
				//			|
				//			|УПОРЯДОЧИТЬ ПО
				//			|	ВкладкаСудСудебнаяСтадияТЧ.ДатаЗаседания УБЫВ";
				//		
				//		Запрос.УстановитьПараметр("Ссылка", СпрВклСудСудебнаяСтадия);
				//		
				//		РезультатЗапроса = Запрос.Выполнить();
				//		
 ...
 Процедура ЗаполнитьТБСписокДокументов (дог)
	     ...
	     // заполню ТБСписокДокументов - 1 -  ЗаявлениеОПроцессуальномПравопреемстве
				...
				// Пройдусь по входящим 
				 ...
				 типДок=Перечисления.ТипДокумента.вх;
					...
					Пока Выборка.Следующий() Цикл
						...
						новаяЗапись.НазваниеДокумента=Выборка.НазваниеДокумента;
...
Функция ДанныеДоговора(дог)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		данныеДог.Вставить("Банк",ВыборкаДетальныеЗаписи.Банк);
...
Процедура СохранитьНаСервере(спр,дог)
	...
	//		 новыйВкладкаИнфСуд=Справочники.ВкладкаСудСудебнаяСтадия.СоздатьЭлемент();
...
Процедура Сохранить(Команда)
		 ...
		 //СохранитьНаСервере(СпрВклСудСудебнаяСтадия,ссылкаДоговор);
		 //       
...
Процедура Документы(Команда)
		 ...
		 //парамСтруктура.Вставить("НазваниеСуда",НазваниеСуда);
		 
		 //парамСтруктура.Вставить("АдресСудаВкладка",АдресСудаВкладка);
		 
		 парамСтруктура.Вставить("Исполнитель",Объект.Исполнитель);
         ...
         ОткрытьФорму("Обработка.ИсполнительноеПроизводство.Форма.ФормаВыборДокумента",парамСтруктура,ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура СохранитьВкладкаИсполнительноеПроизводство(Команда)
	
	
	  СохранитьНаСервереВкладкаИсполнительноеПроизводство(ссылкаВкладкаИсполнительноеПроизводство,ссылкаДоговор);
...
&НаСервере
Процедура СохранитьНаСервереВкладкаИсполнительноеПроизводство(ссылкаСпр,договор)

	     Если НЕ ссылкаСпр.Пустая() Тогда
			 
			 
			 спрОбъект=ссылкаСпр.ПолучитьОбъект();
			 
			 спрОбъект.ЗаменаСтороныУдовлетварено=Объект.ЗаменаСтороныУдовлетварено;
			 спрОбъект.ЗаменаСтороныОтказано=Объект.ЗаменаСтороныОтказано;
			 спрОбъект.ДатаВозбуждения=Объект.ДатаВозбуждения;
			 спрОбъект.ДатаОкончания=Объект.ДатаОкончания;
			 спрОбъект.Статья=Объект.Статья;
			 спрОбъект.ОтделФССП=Объект.ОтделФССП;
			 спрОбъект.Пристав=Объект.Пристав;
			 
			 спрОбъект.Телефон=Объект.ТелефонПристав;
			 
			 спрОбъект.НомерИсполнительногоПроизводства=Объект.НомерИсполнительногоПроизводства;
		 ...
		 Иначе
			 ...
			 спрОбъектНовый=Справочники.ВкладкаИсполнительноеПроизводство.СоздатьЭлемент();
			 
			 спрОбъектНовый.Владелец=договор;
			 спрОбъектНовый.Наименование=договор;
			 
			 спрОбъектНовый.ЗаменаСтороныУдовлетварено=Объект.ЗаменаСтороныУдовлетварено;
			 спрОбъектНовый.ЗаменаСтороныОтказано=Объект.ЗаменаСтороныОтказано;
			 спрОбъектНовый.ДатаВозбуждения=Объект.ДатаВозбуждения;
			 спрОбъектНовый.ДатаОкончания=Объект.ДатаОкончания;
			 спрОбъектНовый.Статья=Объект.Статья;
			 спрОбъектНовый.ОтделФССП=Объект.ОтделФССП;
			 спрОбъектНовый.Пристав=Объект.Пристав;
			 
			 спрОбъектНовый.Телефон=Объект.ТелефонПристав;
			 
			 спрОбъектНовый.НомерИсполнительногоПроизводства=Объект.НомерИсполнительногоПроизводства;
...
&НаКлиенте
Процедура ОтделФССПВкладкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	  СтандартнаяОбработка = Ложь;
	  
	  ПараметрыФормы  = Новый Структура("РежимВыбора, ЗакрыватьПриВыборе", Истина, Истина);
      ОбработкаВыбора = Новый ОписаниеОповещения("ВыбратьЭлементСправочникаОСП", ЭтаФорма);
...
&НаКлиенте
Процедура ВыбратьЭлементСправочникаОСП(РезультатЗакрытия,ДопПараметры) Экспорт
			...
			ОтделФССПВкладка=РезультатЗакрытия.НазваниеОСП;
			Объект.ОтделФССП=РезультатЗакрытия.ссылкаСправочник;
			АдресФССП=РезультатЗакрытия.АдресОСП;
			ОтделФССПстрока=РезультатЗакрытия.НазваниеОСП;
