	  ...
	  ТЗОбработа.Очистить();
	   ...
	   ТаблицаЗначений = ПЗ.Результат.Выгрузить();
	    ...
	    Для каждого стр Из ТаблицаЗначений Цикл
			...
			ПервоначальныйКредитор=Неопределено;
			
			ДатаВынесенияРешения=Неопределено;
			
			
			ПервоначальныйНомерКредитногоДоговора=Неопределено;
			...
			КонецЕсли;
			 ...
			 Если ТаблицаЗначений.Колонки.Найти("ПервоначальныйКредитор") <> Неопределено Тогда
				 
				 
				 Если стр.первоначальныйКредитор="" Тогда
					 ПервоначальныйКредитор="";
					 
				 Иначе   	
					   ПервоначальныйКредитор= НайтиБанк(стр.первоначальныйКредитор);
			...
			Если ТаблицаЗначений.Колонки.Найти("ДатаВынесенияРешения") <> Неопределено Тогда
				
				      ДатаВынесенияРешения=СтроковыеФункцииКлиентСервер.СтрокаВДату(стр.ДатаВынесенияРешения);
				  
			КонецЕсли;

			
			Если ТаблицаЗначений.Колонки.Найти("ПервоначальныйНомерКредитногоДоговора") <> Неопределено Тогда
				 
				    ПервоначальныйНомерКредитногоДоговора=стр.ПервоначальныйНомерКредитногоДоговора;
			...
			новаяСтрока.ПервоначальныйКредитор= ПервоначальныйКредитор;
			
			новаяСтрока.ДатаВынесенияРешения=ДатаВынесенияРешения;
			
			новаяСтрока.ПервоначальныйНомерКредитногоДоговора=ПервоначальныйНомерКредитногоДоговора;
...
&НаСервере
Функция НайтиБанк(первоначальныйКредитор)
	       ...
	       // по полному названию
			...
			Запрос.Текст = 
				...
				|	Банки.ПолноеНазвание КАК ПолноеНазвание
				|ИЗ
				|	Справочник.Банки КАК Банки
				|ГДЕ
				|	Банки.ПолноеНазвание = &ПолноеНазвание";
			
			Запрос.УстановитьПараметр("ПолноеНазвание", первоначальныйКредитор);
			...
			Если ВыборкаДетальныеЗаписи.Количество()>0 Тогда
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					ссылкаБанк=ВыборкаДетальныеЗаписи.Ссылка;
			...
			Иначе
				...
				//  по наименованию
						...
						Запрос.УстановитьПараметр("Наименование", первоначальныйКредитор);
...
&НаСервере
Процедура УстановитьПервичныйНомерКрДоговора(договор,первНомер, первБанк)

	       спрДоговоры=договор.ПолучитьОбъект();   
		   
		   спрДоговоры.НомерПервичногоКредитногоДоговора=первНомер;
		   
		   Если ЗначениеЗаполнено(первБанк) Тогда
		   
		   	     спрДоговоры.БанкПервоначальныйКредитор=первБанк;
...
Процедура ЗагрузитьНаСервере()
	 ...
	 Попытка
		  ...
		  Для каждого стр Из ТЗОбработа Цикл
			  ...
			  // Проверить  и записать Первоначальный номер КД
			  Если стр.ПервоначальныйНомерКредитногоДоговора<>"" Тогда
				  
				  Если  ЗначениеЗаполнено(стр.Договор) Тогда           
					  
										  
					  УстановитьПервичныйНомерКрДоговора(стр.Договор,стр.ПервоначальныйНомерКредитногоДоговора,стр.ПервоначальныйКредитор);
			 ...
			 Если ссылкаСправочник=Неопределено Тогда 
			        ...
			        новаяСтрока=Справочники.СостояниеДелБанкДолжник.СоздатьЭлемент();
					 ...
					 новаяСтрока.ПервоначальныйНомерКредитногоДоговора=стр.ПервоначальныйНомерКредитногоДоговора;
					 
					 новаяСтрока.ДатаВынесенияРешения=стр.ДатаВынесенияРешения;
					 
					 новаяСтрока.ПервоначальныйКредитор=стр.ПервоначальныйКредитор;
			 ...
			 Иначе
			     ...
			     об=ссылкаСправочник.ПолучитьОбъект();
			      ...
			      Если стр.ДатаЗапроса>об.ДатаЗапроса Тогда
					 ...
					 об.ПервоначальныйНомерКредитногоДоговора=стр.ПервоначальныйНомерКредитногоДоговора;
					 
					 об.ДатаВынесенияРешения=стр.ДатаВынесенияРешения;
					 
					  об.ПервоначальныйКредитор=стр.ПервоначальныйКредитор;
