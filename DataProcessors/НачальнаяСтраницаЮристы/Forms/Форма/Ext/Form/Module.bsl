...
&НаКлиенте
Процедура СохранитьВыпискаИзРеестраВPDF(строкаПуть,имяФайлаВстр,расширФайла,стрПодкаталогПакета,стрДоговор)
         ...
         MSWord = новый COMОбъект("Word.Application");
			  ...
			  //Передаем текущие параметры форм в MSWord
				 ...
				 Если  MSWordDoc.Bookmarks.Exists("ПодписьПр")  Тогда
					 ...
					 КонецЕсли;
					  ...
					  имяД="Выписка из Акта уступки прав (требований)";
		  ...
		  Исключение
				...
				Сообщить("Ошибка-----_ВыпискаИзРеестра - договор "+стрДоговор);
				ТекстОшибки = ОписаниеОшибки();
				Сообщить(ТекстОшибки);
			 ...
			 КонецЕсли;
			  ...
			  Для каждого стр  Из  ТБРеестрДляОтправки Цикл
					  ...
					  Если срДляПечати.названиеСправочника="ЗаявлениеОПроцессуальномПравопреемстве" Тогда
									  ...
									  стрДоговор=Строка(стр.КредитныйДоговор);
		    		    			  ...
		    		    			  Для каждого стрМассив  Из стрФайлыМассив  Цикл
											 ...
											 Если инд>0 Тогда
				                                  ...
				                                  СохранитьВыпискаИзРеестраВPDF(строкаПуть,имяД,расширФайла,стрПодкаталогПакета,стрДоговор);
	   ...
	   стрТек=Элементы.ТБРеестрДляОтправки.ТекущиеДанные.ДокументСсылка;
			   ...
			   Если стрТек<>Неопределено Тогда
				   ...
				   Для каждого стрИнд Из Элементы.ТБРеестрДляОтправки.ВыделенныеСтроки Цикл
							  ...
							  структураДанные.Вставить("ссылкаДок",стр.ДокументСсылка);
							  структураДанные.Вставить("идентификаторСтр",стрИнд);
							     			   
							  
							   массивУдалить.Добавить(структураДанные);
				  ...
				  Оповещение = Новый ОписаниеОповещения("ПослеОтветаНаВопросУдалитьИзТБОтправка",ЭтаФорма,массивУдалить);
...
Процедура ПослеОтветаНаВопросУдалитьИзТБОтправка(Результат, Параметры) Экспорт 
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		УдалитьПоДокуменСссылка(Параметры);

		Для каждого стрМассив  Из Параметры Цикл
		    СтрокаТаблицы = ТБРеестрДляОтправки.НайтиПоИдентификатору(стрМассив.идентификаторСтр);
...
&НаСервере
Процедура УдалитьПоДокуменСссылка(ссылкаДокМассив)

	
	Для каждого стрМассив Из ссылкаДокМассив Цикл
		
	 об=стрМассив.ссылкаДок.ПолучитьОбъект();
			    ...
			    Для каждого стрИнд Из Элементы.СостояниеДел.ВыделенныеСтроки Цикл
					 ...
					 стр= Элементы.СостояниеДел.ДанныеСтроки(стрИнд);
					  ...
					  данныеГруппаДоговоровСтруктура=Новый Структура;
							   ...
							   данныеГруппаДоговоровСтруктура.Вставить("ДатаДела",стр.ДатаВынесенияРешения);
...
Процедура ПослеОтветаНаВопросСохранитьГАС(Результат, Параметры) Экспорт 
			  ...
			  Если срДляПечати.названиеСправочника="ЗаявлениеОПроцессуальномПравопреемстве" Тогда
				      ...
				      стрФайлыМассив= ПолучитьПрисоединенныеФайлы(срДляПечати.ссылкаДокПечати);
						...
						СоздатьPDFфайлСПодписью(срДляПечати.ссылкаДокПечати,стрПодкаталогПакета);		   
					     ...
					     новаяСтр=ТЗОбращениеВГАС.Добавить();
						 новаяСтр.НомерДоговора=ВернутьНомерДоговора(стр.КредитныйДоговор);
								   новаяСтр.ФИО=Строка(стр.ФИОДолжника);
								   новаяСтр.НазваниеДокумента="Заявление О Процессуальном Правопреемстве";
								   новаяСтр.GUID=стр.GUID;
	 ...
	 КонецЕсли;
	  ...
	  //сформировать файл Обращения в ГАС
	  Если ТЗОбращениеВГАС.Количество()>0 Тогда
		  
		  ТабДокумент= ПолучитьТабличныйДокументОбращениеГАС();
		  
		  имяфайлОбращение=путьКаталог+"\ОбращенияВГАС.XLSX";  
		  
		  ТабДокумент.Записать(имяфайлОбращение,ТипФайлаТабличногоДокумента.XLSX);
...
&НаСервере
Функция ПолучитьТабличныйДокументОбращениеГАС()
				    ...
				    Макет =  ОтчетОбъект.ПолучитьМакет("ОбращениеВГАС"); 
					...
					Для каждого Стр  Из ТЗОбращениеВГАС  Цикл
							
							ОбластьСтрока=Макет.ПолучитьОбласть("Строка");
							
							ОбластьСтрока.Параметры.номер=инд;
							...
							ОбластьСтрока.Параметры.GUID=Стр.GUID;
...
&НаСервере
Функция ВернутьНомерДоговора (ссылкаДог)

	  об=ссылкаДог.ПолучитьОбъект();
	  
	  Возврат об.НомерДоговора; 
