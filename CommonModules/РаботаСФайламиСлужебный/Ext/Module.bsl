...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Обработчик.ИзменяемыеОбъекты    = ИзменяемыеОбъектыПриПереносеЭлектронныхПодписейИСертификатовШифрования();
		...
		Приоритет.Порядок = "Любой";
		Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
		Приоритет.Процедура = "РаботаСФайламиСлужебный.ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы";
		Приоритет.Порядок = "Любой";
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "2.4.1.62";
		Обработчик.Комментарий =
			НСтр("ru = 'Перенос электронных подписей с версий файлов на файлы.'");
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("d0a26f83-8dd3-4965-93a6-8469c9c91998");
		Обработчик.Процедура = "РаботаСФайламиСлужебный.ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.ОчередьОтложеннойОбработки = 2;
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "РаботаСФайламиСлужебный.ЗарегистрироватьОбъектыДляПереносаЭлектронныхПодписейСВерсийФайловНаФайлы";
		Обработчик.ЧитаемыеОбъекты      = ОбъектыПриПереносеЭлектронныхПодписейСВерсийФайловНаФайлы(Истина);
		Обработчик.ИзменяемыеОбъекты    = ОбъектыПриПереносеЭлектронныхПодписейСВерсийФайловНаФайлы(Ложь);
		Обработчик.ПроцедураПроверки    = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
		Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
		Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
		Приоритет.Процедура = "РегистрыСведений.ДвоичныеДанныеФайлов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
...
Процедура ИзвлечьТекстИзФайла(ФайлБезТекста, ФайлЗаблокирован, ИмяФайлаСДвоичнымиДанными)
	...
	Попытка
		...
		Если ФайлОбъект <> Неопределено Тогда
			...
			УдалятьВременныйФайл = ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе 
				И Не ПустаяСтрока(ИмяФайлаСДвоичнымиДанными);
...
Функция ОбъектыПриПереносеЭлектронныхПодписейСВерсийФайловНаФайлы(Читаемые)
