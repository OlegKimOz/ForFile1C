...
// Параметры
//  ИмяФайла  	– Строка
//  файлДанных	– Excel.Application – Сюда возвращается ссылка на объект Excel
//  Ошибка		– Строка - Если произошла ошибка, возвращается её описание
//
// Возвращаемое значение:
//   Булево 	– Если файл открыт - истина
//
Функция ОткрытьФайлДанных(ИмяФайла, ФайлДанных, Ошибка = "") Экспорт
    Перем ФД;
	...
	Возврат Истина;
КонецФункции // ОткрытьФайлДанных()
...
Процедура ДобавитьТЧкОбщейТаблице(ОбщаяТаблица, ТЧ, НазваниеТЧ)
	...
	Для каждого СтрокаТЧ Из ТЧ Цикл
		...
		Иначе
			...
			//МассивЗаголовков = глРазложитьСтрокуВМассивПодстрок(Заголовки, "#"); //#МРС не знаю от куда это....    //Раскоментить
...
Функция ПроверитьСоответствиеШаблону(ФайлДанных, ШаблонСсылка, ТаблицаКолонок) Экспорт
	...
	Пока СокрЛП(Строка(ФайлДанных.Cells(СтрокаЗаголовокФайла, ТекКолонка).Value)) <> "" Цикл
		Заголовок = СвернутьТекст(Строка(ФайлДанных.Cells(СтрокаЗаголовокФайла, ТекКолонка).Value));

		// Для не пустого заголовка
		Если Заголовок <> "" Тогда
			ПроставитьНомерКолонкиВТаблице(Заголовок, ТекКолонка, ОбщаяТаблица);
		КонецЕсли; 
...
Процедура ДобавитьДанныеВТаблицу_СтарыйВариант(Таблица, Значение, Реквизит, Поиск = Ложь)
...
Функция СоздатьТаблицуРеквизитов(Вид, Шаблон = Неопределено) Экспорт
	Перем Таблица;
...
// В функцию нужно передать таблицу со значениями реквизитов
// если Создавать = Истина и элемент не найден
// будет создан новый и заполнен значениями из таблицы
Функция НайтиЭлементСправочника(Таблица, Вид, Создавать = Ложь, Найден = "") Экспорт
	Перем Элемент, ПараметрыОтбора, Условие;
	...
	Если Результат.Количество() = 0 Тогда
		Если Создавать Тогда
			...
			Элемент = Справочники[Вид].СоздатьЭлемент();
			...
			Для каждого СтрокаТЗ Из Таблица Цикл
				...
				Попытка
					Если Вид = "Адреса" И СтрокаТЗ.Реквизит = "Регион" Тогда
						...
						КонецЕсли;
						Элемент[СтрокаТЗ.Реквизит] = НайденныйРегион;
					Иначе	
						Элемент[СтрокаТЗ.Реквизит] = СтрокаТЗ.Значение;
					КонецЕсли;
				Исключение
				КонецПопытки;
			...
			Если (Вид = "Адреса") ИЛИ (Вид = "Телефоны") Тогда
			    Элемент.Тип = Таблица[0].Тип;
			КонецЕсли; 
			Если (Вид = "ДополнительныеДанные") Тогда
				Элемент.Реквизит = Таблица[0].Тип;
				Элемент.Владелец = ВладелецП;
				Элемент.Наименование = Таблица[0].Значение; // +Крылов 2011.02.04 СтрокаТЗ.Значение; 
				СпрПров = Справочники.ДополнительныеДанные;
				ТекДоп = СпрПров.НайтиПоНаименованию(СокрЛП(Элемент.Наименование),Истина,,ВладелецП);
				Если ТекДоп = Справочники.ДополнительныеДанные.ПустаяСсылка() Тогда
					Элемент.Записать();
					Элемент = Элемент.Ссылка;
				Иначе
					Найден = Истина;
					Элемент = ТекДоп.Ссылка;
				КонецЕсли;
			Иначе
				Элемент.Записать();
				Элемент = Элемент.Ссылка;
		...
		Иначе
			Элемент = Справочники[Вид].ПустаяСсылка();
			Найден = Ложь;
		КонецЕсли; 
	Иначе
		Элемент = Результат[0].ИскомыйЭлемент;
		// +Крылов 2011.02.11
		ОбновлениеДанныхОбъекта(Элемент, Таблица);
		// -Крылов 2011.02.11
		Найден = Истина;
	КонецЕсли; 
...
Функция НайтиЭлементСправочникаПоНаименованию(Вид, Наименование, Создавать = Ложь, Найден = "") Экспорт
	Перем тз;
	...
	Возврат НайтиЭлементСправочника(Таблица, Вид, Создавать, Найден);
...
Функция ЗаписатьДанныеДолжника(ДанныеДолжника, Найден = "") Экспорт
	Перем Должник;
	...
	// Возвращаем ссылку на должника
	Возврат Должник;
КонецФункции // ЗаписатьДанныеДолжника()

// (1) Должники
Функция ЗагрузитьДанные_СписокДолжников(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ТаблицаДолжников)
	...
	Пока ВсяСтрока <> "" Цикл
        ...
        Для каждого СтрокаТЗ Из ТаблицаКолонок Цикл
			КолонкаФайла = СтрокаТЗ.НомерКолонки;
			
			Значение = СокрЛП(Строка(ФайлДанных.Cells(ТекСтрока, КолонкаФайла).Value));
			Если СтрокаТЗ.Реквизит = "Фамилия" Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если СтрокаТЗ.Реквизит = "Имя" Тогда
				КолПроб = СтрЧислоВхождений(Значение," ");
				Если КолПроб > 0 Тогда
					НомПроб = Найти(Значение," ");
				...
				КонецЕсли;
			КонецЕсли;
			Если СтрокаТЗ.Реквизит = "Отчество" Тогда
				КолПроб = СтрЧислоВхождений(Значение," ");
				Если КолПроб > 0 Тогда
					НомПроб = Найти(Значение," ");
					Значение = СокрЛП(Сред(Значение,НомПроб));
				...
				КонецЕсли;
			КонецЕсли;
			Если СтрокаТЗ.Реквизит = "НомерПаспорта" Тогда
				НомерПаспорта = "";
				ТекСтр = Значение;
				НомСтр = 0;
				Пока НомСтр <= 50 Цикл
			...
			Если Значение = "" Тогда
				Продолжить;	
			ИначеЕсли СтрокаТЗ.Реквизит = "ДатаРождения" Тогда
				Попытка
			...
			Элемент = СписокНеобходимыхРеквизитов.НайтиПоЗначению(СтрокаТЗ.ТаблицаШаблона + "." + СтрокаТЗ.Реквизит);
			Если Элемент <> Неопределено Тогда
				Элемент.Пометка = Истина;
			КонецЕсли; 
			
			Если СтрокаТЗ.ТаблицаШаблона = "Должники" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДолжника, Значение, СтрокаТЗ.Реквизит);
			КонецЕсли; 
		КонецЦикла; 
...
// (1.2) Должники
Функция ЗагрузитьДанные_СписокДолжниковПоДоговору(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ТаблицаДолжников, ПробоватьИсправитьНомерДоговора = 0)
	Перем ДанныеДоговора;
	ТаблицаДолжников = Новый ТаблицаЗначений;
	ТаблицаДолжников.Колонки.Добавить("Должник"); // Элемент справочника
	...
	Пока ВсяСтрока <> "" Цикл
        ...
        Для каждого СтрокаТЗ Из ТаблицаКолонок Цикл
			...
			Если СтрокаТЗ.Реквизит = "НомерДоговора" Тогда
				Значение = СтрЗаменить(Значение, " ", "");
				Значение = СтрЗаменить(Значение, Символ(160), "");
				ТекНомерДоговора = Значение;
			ИначеЕсли СтрокаТЗ.Реквизит = "ДатаОтзыва" Тогда
				Попытка
					Значение = Лев(Значение, 10);
					Значение = Дата(Прав(Значение, 4) + Сред(Значение, 4, 2) + Лев(Значение, 2));
				Исключение
			...
			Если СтрокаТЗ.ТаблицаШаблона = "Договоры" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДоговора, Значение, СтрокаТЗ.Реквизит);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "ДополнительныеКолонки" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДополнительныеКолонки, Значение, СтрокаТЗ.Реквизит);
			КонецЕсли; 
		КонецЦикла; 
		...
		Если Записывать Тогда
			Найден = "";
		    ТекущийДоговор = НайтиЭлементСправочника(ДанныеДоговора, "Договоры", Ложь, Найден);
			...
			Иначе
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = "ВНИМАНИЕ! Не найден договор: " + ТекНомерДоговора; 
				Сообщение.Сообщить();

				//Сообщить("ВНИМАНИЕ! Не найден договор: " + ТекНомерДоговора);
			КонецЕсли; 
		Иначе
			ТекущийДолжник = Неопределено;
		КонецЕсли;   
...
Функция ЧтениеДанных(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ТаблицаКонтактов, ЗагрузкаДанныхПоНомеруДоговора = Ложь)
	...
	Пока НЕ ПустаяСтрока(ВсяСтрока) Цикл
        ...
        Для каждого СтрокаТЗ Из ТаблицаКолонок Цикл
			...
			// +Крылов 2011.02.04
			Значение = ?(ПустоеЗначение(Значение),"",Значение);
			// -Крылов 2011.02.04
			...
			Если СтрокаТЗ.Реквизит = "ВсегоЗадолженность" Тогда
				Значение = СтрЗаменить(Значение, "-", "");
			ИначеЕсли СтрокаТЗ.Реквизит = "ТипКредита" Тогда
				Значение = НайтиЭлементСправочникаПоНаименованию("ТипыКредитов", Значение, Истина);
			ИначеЕсли СтрокаТЗ.Реквизит = "Валюта" Тогда
				// +Крылов 2011.02.04
				Значение = СтрокуВВалюту(Значение);
				//Значение = НайтиЭлементСправочникаПоНаименованию("Валюты", Значение, Истина);
				// -Крылов 2011.02.04
			ИначеЕсли СтрокаТЗ.Реквизит = "ДатаРождения" 
				ИЛИ СтрокаТЗ.Реквизит = "ДатаФинансирования" 
				ИЛИ СтрокаТЗ.Реквизит = "ДатаОкончанияДоговора" 
				ИЛИ СтрокаТЗ.Реквизит = "ДатаВозникновенияДолга" 
				ИЛИ СтрокаТЗ.Реквизит = "ПаспортКогдаВыдан" 
				ИЛИ СтрокаТЗ.Реквизит = "ДатаЕжемесячногоПлатежа" 
				ИЛИ СтрокаТЗ.Реквизит = "ДатаРасчетаЗадолженности" Тогда
				Попытка
			...
			Если СтрокаТЗ.ТаблицаШаблона = "Должники" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДолжника, Значение, СтрокаТЗ.Реквизит);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Договоры" Тогда
				//НадоДоговор = Истина;
				ДобавитьДанныеВТаблицу(ДанныеДоговора, Значение, СтрокаТЗ.Реквизит);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Адреса" Тогда
				ДобавитьДанныеВТаблицу(ДанныеАдресов, Значение, СтрокаТЗ.Реквизит,, СтрокаТЗ.Тип);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Телефоны" Тогда
				// +Крылов 2011.02.04
				Значение = СтрокуВПредставлениеТелефона(Значение);
				// -Крылов 2011.02.04
				ДобавитьДанныеВТаблицу(ДанныеТелефонов, Значение, СтрокаТЗ.Реквизит,, СтрокаТЗ.Тип);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Реестр" Тогда
				//НадоРеестр = Истина;
				ДобавитьДанныеВТаблицу(ДанныеРеестра, Значение, СтрокаТЗ.Реквизит);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Контакты" Тогда
				НовСтр = ТаблицаКонтактов.Добавить();
				НовСтр.Комментарий = СокрЛП(Значение);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "ДополнительныеКолонки" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДополнительные, Значение, СтрокаТЗ.Реквизит, , СтрокаТЗ.Реквизит);
			КонецЕсли;
		КонецЦикла; 
...
Процедура ЗаписьДанных(ТаблицаСДанными, ЗагрузкаДанныхПоНомеруДоговора = Ложь, НеЗагружатьВРеестрПриНаличииДоговора = Ложь)
	...
	Для Каждого ТекДанные Из ТаблицаСДанными Цикл
		...
		Если НЕ ЗагрузкаДанныхПоНомеруДоговора Тогда
			// Искать по НомеруПаспорта
			НайденныеОбъектыПоНомеруПаспорта = НайтиСправочникПоРеквизитам("Должники", 
				Новый Структура("НомерПаспорта",ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника, "НомерПаспорта")));
			Если НайденныеОбъектыПоНомеруПаспорта.Количество() = 0 Тогда
				
				// Искать по ФИО
				НайденныеОбъектыПоФИО = НайтиСправочникПоРеквизитам("Должники", 
					Новый Структура("Фамилия, Имя, Отчество",
						ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника,"Фамилия"), ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника,"Имя"), ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника,"Отчество")));
				Если НайденныеОбъектыПоФИО.Количество() = 0 Тогда
					Должник = СоздатьЭлемент(Справочники.Должники, ТекДанные.ДанныеДолжника);
				Иначе
					// Искать в найденных по дате рождения
					НайденныеОбъектыПоФИОиДатаРождения = НайтиСправочникПоРеквизитам(НайденныеОбъектыПоФИО, 
						Новый Структура("ДатаРождения",ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника, "ДатаРождения")));
			...
			Иначе
				...
				// Искать в найденных по ФИО
				НайденныеОбъектыПоНомеруПаспортаФИО = НайтиСправочникПоРеквизитам(НайденныеОбъектыПоНомеруПаспорта, 
					Новый Структура("Фамилия, Имя, Отчество",
						ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника,"Фамилия"), ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника,"Имя"), ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДолжника,"Отчество")));
				Если НайденныеОбъектыПоНомеруПаспортаФИО.Количество() = 0 Тогда
					// необходима доп. проверка
			...
			КонецЕсли;
		Иначе
			ТЗ_Договоры = НайтиСправочникПоРеквизитам("Договоры", 
					Новый Структура("НомерДоговора",
						ПолучитьЗначениеРеквизитаИзТаблицы(ТекДанные.ДанныеДоговора,"НомерДоговора")));
	...
	Для Каждого ТекПроверка Из ТЗ_ДопПроверка Цикл
		НоваяСтрока = Форма.Данные.Добавить();
		НоваяСтрока.Ссылка 				= ТекПроверка.Ссылка;
		НоваяСтрока.Фамилия 			= ТекПроверка.Ссылка.Фамилия;
		НоваяСтрока.Имя 				= ТекПроверка.Ссылка.Имя;
		НоваяСтрока.Отчество 			= ТекПроверка.Ссылка.Отчество;
		НоваяСтрока.НомерПаспорта 		= ТекПроверка.Ссылка.НомерПаспорта;
		НоваяСтрока.ДатаРождения 		= ТекПроверка.Ссылка.ДатаРождения;
		
		НоваяСтрока.ФамилияФайл				= ПолучитьЗначениеРеквизитаИзТаблицы(ТекПроверка.Данные.ДанныеДолжника,"Фамилия");
		НоваяСтрока.ИмяФайл 				= ПолучитьЗначениеРеквизитаИзТаблицы(ТекПроверка.Данные.ДанныеДолжника,"Имя");
		НоваяСтрока.ОтчествоФайл 			= ПолучитьЗначениеРеквизитаИзТаблицы(ТекПроверка.Данные.ДанныеДолжника,"Отчество");
		НоваяСтрока.НомерПаспортаФайл 		= ПолучитьЗначениеРеквизитаИзТаблицы(ТекПроверка.Данные.ДанныеДолжника,"НомерПаспорта");
		НоваяСтрока.ДатаРожденияФайл 		= ПолучитьЗначениеРеквизитаИзТаблицы(ТекПроверка.Данные.ДанныеДолжника,"ДатаРождения");
		...
		Для Каждого ТекАдр ИЗ ТЗ_Адреса Цикл
			НоваяСтрока.Адреса 		= НоваяСтрока.Адреса + Строка(ТекАдр.Тип) + ": " + ТекАдр.Значение + "; ";
		КонецЦикла;
		
		НоваяСтрока.АдресаФайл		= "";
		Для Каждого ТекАдр ИЗ ТекПроверка.Данные.ДанныеАдресов Цикл
			НоваяСтрока.АдресаФайл 		= НоваяСтрока.АдресаФайл + Строка(ТекАдр.Тип) + ": " + ТекАдр.Значение + "; ";
		КонецЦикла;
	...
	Для Каждого СтрРез Из Результат Цикл
		...
		ДопПроверкаТекСтрока = ТЗ_ДопПроверка[НомерСтроки-1];
		Если СтрРез.Соответствие Тогда
			ДопПроверкаТекСтрока.Ссылка = СтрРез.Ссылка;
			ДопПроверкаТекСтрока.Данные.Должник = СтрРез.Ссылка;
			...
			Если НЕ ИзменятьНомерПаспорта И НЕ ПустаяСтрока(СтрРез.НомерПаспорта) И СтрРез.НомерПаспортаФайл <> СтрРез.НомерПаспорта Тогда
				...
				ФормаВопрос = ПолучитьОбщуюФорму("ДаНетДляВсех");
				ФормаВопрос.ТекстВопроса = "Для должника " + Строка(СтрРез.Ссылка) + " номер паспорта в базе и в файле отличается.
					|Записать в базу номер паспорта из файла?";
				Ответ = ФормаВопрос.ОткрытьМодально();
				
				Если Ответ = "ДляВсех" Тогда
					ИзменятьНомерПаспорта = Истина;
				ИначеЕсли Ответ = "Да" Тогда	
					ДопПроверкаТекСтрока.Данные.ДанныеДолжника.Найти("НомерПаспорта").Значение = СтрРез.НомерПаспортаФайл;
					ОбъектДолжник = СтрРез.Ссылка.ПолучитьОбъект();
					ОбъектДолжник.НомерПаспорта = СтрРез.НомерПаспортаФайл;
		...
		Иначе
			НовыйДолжник = СоздатьЭлемент(Справочники.Должники, ДопПроверкаТекСтрока.Данные.ДанныеДолжника);
		КонецЕсли;
	...
	Для Каждого ТекДанные Из ТаблицаСДанными Цикл
		...
		ИначеЕсли НеЗагружатьВРеестрПриНаличииДоговора Тогда
			ТекДанные.ЗагружатьВРеестр = Справочники.Договоры.НайтиПоРеквизиту("НомерДоговора",ПолучитьЗначениеРеквизитаИзТаблицы(ТекПроверка.Данные.ДанныеДоговора,"НомерДоговора"),,СтрРез.Ссылка).Пустая();
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ОбновлениеДанныхОбъекта(Ссылка, Данные)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Для каждого ТЗ Из Данные Цикл
			...
			Объект[ТЗ.Реквизит] = ТекЗначение;
		КонецЦикла; 
		Объект.Записать();
	КонецЕсли;	
КонецПроцедуры

Процедура ОбработкаДанных(ТаблицаСДанными, Банк, Ответственный, Документ, ДатаЗарузки, ТаблицаКонтактов, ЗагрузкаДанныхПоНомеруДоговора = Ложь)
	ТекСтрока = 0;
	Для Каждого ТекДанные Из ТаблицаСДанными Цикл
		ТекСтрока = ТекСтрока + 1;
		...
		Для каждого ТЗ Из МассивТЗ Цикл
			ДобавитьДанныеВТаблицу(ТЗ, ТекущийДолжник, "Владелец", Истина, ТЗ[0].Тип);
			Если НЕ ПустоеЗначение(ТЗ[0].Значение) Тогда  // +Крылов 2011.02.04
...
Функция ЗагрузитьДанные_ЗагрузкаРеестра(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗарузки, Банк,Ответственный, ЗагрузкаДанныхПоНомеруДоговора = Ложь, Реестр = Неопределено, НеЗагружатьВРеестрПриНаличииДоговора = Ложь)
	
	НачатьТранзакцию();

	Если НЕ ЗагрузкаДанныхПоНомеруДоговора Тогда
		Документ = ?(НЕ ЗначениеЗаполнено(Реестр), Документы.Реестр.СоздатьДокумент(), Реестр.ПолучитьОбъект());         
		Документ.Банк = Банк;
	...
	ТаблицаСДанными = ЧтениеДанных(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ТаблицаКонтактов, ЗагрузкаДанныхПоНомеруДоговора);
	...
	ОбработкаДанных(ТаблицаСДанными, Банк, Ответственный, Документ, ДатаЗарузки, ТаблицаКонтактов, ЗагрузкаДанныхПоНомеруДоговора);         

	Если НЕ ЗагрузкаДанныхПоНомеруДоговора Тогда
		Попытка
			Если Документ.Должники.Количество() = 0 Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = "Рееестр не создан. Отсутствуют загруженные должники."; 
				Сообщение.Сообщить();

				//Сообщить("Рееестр не создан. Отсутствуют загруженные должники.");
			Иначе
				Документ.Дата = ДатаЗарузки;	
				Документ.Записать();
				Сообщение = Новый СообщениеПользователю;
...
Функция ЗагрузитьДанные_ЗагрузкаКонтакта(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗарузки, Банк,Ответственный)
    Перем Документ;
	...
	Пока НЕ ПустаяСтрока(ВсяСтрока) Цикл
		...
		Для каждого СтрокаТЗ Из ТаблицаКолонок Цикл
			...
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Телефоны" Тогда
				ДобавитьДанныеВТаблицу(ДанныеТелефонов, Значение, СтрокаТЗ.Реквизит,, СтрокаТЗ.Тип);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Контакт" Тогда
				НадоКонтакт = Истина;
				ДобавитьДанныеВТаблицу(ДанныеКонтактД, Значение, СтрокаТЗ.Реквизит);
			КонецЕсли;
		КонецЦикла; 
		...
		Если Записывать Тогда
			...
			Если ТекКлиент = Справочники.Должники.ПустаяСсылка() Тогда
				...
				Если Результат.Количество() > 0 Тогда
					...
					Для Каждого СтрРез Из Результат Цикл
						...
						КонецЕсли;
						//#МРС
						//Вот это надо вынести кудато в настройки!
						//Данные операции необходимо проводить на сервере, но задать вопрос отсюда не получится.
						
						//Сообщить("У клиента "+Должник+" паспорт № "+СокрЛП(Должник.НомерПаспорта) + " не совпадают с загружаемым "+СокрЛП(НомерПаспорта));
						//Если Вопрос("Исправить?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
						//#МРС
						ТекДолжник = Должник.ПолучитьОбъект();
						ТекДолжник.НомерПаспорта = НомерПаспорта;
						ТекДолжник.Записать();
			...
			//#МРС не используется в сервернойна стороне сервера, если это критично то лучше выводить в сообщение пользователю.
			//Состояние(Строка(ТекСтрока) + ") Добавлен " + Строка(ТекущийДолжник));
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = Строка(ТекСтрока) + ") Добавлен " + Строка(ТекущийДолжник); 
			Сообщение.Сообщить();
			//#МРС
...
Функция ЗагрузитьДанные_ОбновлениеРеестра(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗарузки)
    Перем Документ;
	...
	Пока ВсяСтрока <> "" Цикл
        ...
        Для каждого СтрокаТЗ Из ТаблицаКолонок Цикл
			...
			Если СтрокаТЗ.ТаблицаШаблона = "Договоры" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДоговора, Значение, СтрокаТЗ.Реквизит);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "ОбновлениеРеестра" Тогда
				ДобавитьДанныеВТаблицу(ДанныеРеестра, Значение, СтрокаТЗ.Реквизит);
			КонецЕсли;
		КонецЦикла; 
...
// (4) Загрузка платежей
Функция ЗагрузитьДанные_ЗагрузкаПлатежей(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗарузки)
    Перем Документ;
	...
	Пока ВсяСтрока <> "" Цикл
        ...
        Для каждого СтрокаТЗ Из ТаблицаКолонок Цикл
			...
			Если СтрокаТЗ.Реквизит = "ВсегоЗадолженность" Тогда
				Значение = СтрЗаменить(Значение, "-", "");
			ИначеЕсли СтрокаТЗ.Реквизит = "ДатаПлатежа" Тогда
				Попытка
					Значение = Лев(Значение, 10);
					Значение = Дата(Прав(Значение, 4) + Сред(Значение, 4, 2) + Лев(Значение, 2));
				Исключение
			...
			Если СтрокаТЗ.ТаблицаШаблона = "Договоры" Тогда
				ДобавитьДанныеВТаблицу(ДанныеДоговора, Значение, СтрокаТЗ.Реквизит);
			ИначеЕсли СтрокаТЗ.ТаблицаШаблона = "Платежи" Тогда
				ДобавитьДанныеВТаблицу(ДанныеПлатежи, Значение, СтрокаТЗ.Реквизит);
			КонецЕсли;
		КонецЦикла; 
...
// Выбор шаблона
&НаСервере
Функция ЗагрузитьДанныеИзФайла(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗагрузки = "", ТаблицаДолжников = "", ПробоватьИсправитьНомерДоговора = 0, Банк,Ответственный = "", ЗагрузкаДанныхПоНомеруДоговора = Ложь, Реестр = Неопределено, НеЗагружатьВРеестрПриНаличииДоговора = Ложь) Экспорт
	...
	Если ШаблонСсылка.НазначениеШаблона = Перечисления.НазначенияШаблонов.СписокДолжников Тогда
		ТаблицаДолжников = Новый ТаблицаЗначений;
		Возврат ЗагрузитьДанные_СписокДолжников(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ТаблицаДолжников);
		
	ИначеЕсли ШаблонСсылка.НазначениеШаблона = Перечисления.НазначенияШаблонов.СписокДолжниковПоДоговору Тогда
		ТаблицаДолжников = Новый ТаблицаЗначений;
		Возврат ЗагрузитьДанные_СписокДолжниковПоДоговору(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ТаблицаДолжников, ПробоватьИсправитьНомерДоговора);
		
	ИначеЕсли ШаблонСсылка.НазначениеШаблона = Перечисления.НазначенияШаблонов.ЗагрузкаРеестра Тогда
	    Возврат ЗагрузитьДанные_ЗагрузкаРеестра(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗагрузки, Банк,ОтветственныйТ, ЗагрузкаДанныхПоНомеруДоговора, Реестр, НеЗагружатьВРеестрПриНаличииДоговора);
		
	ИначеЕсли ШаблонСсылка.НазначениеШаблона = Перечисления.НазначенияШаблонов.ЗагрузкаКонтакта Тогда
	    Возврат ЗагрузитьДанные_ЗагрузкаКонтакта(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗагрузки, Банк,ОтветственныйТ);
		
	ИначеЕсли ШаблонСсылка.НазначениеШаблона = Перечисления.НазначенияШаблонов.ОбновлениеРеестра Тогда
	    Возврат ЗагрузитьДанные_ОбновлениеРеестра(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗагрузки);
		
	ИначеЕсли ШаблонСсылка.НазначениеШаблона = Перечисления.НазначенияШаблонов.ЗагрузкаПлатежей Тогда
	    Возврат ЗагрузитьДанные_ЗагрузкаПлатежей(ФайлДанных, ШаблонСсылка, ТаблицаКолонок, ДатаЗагрузки);
		
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Для данного шаблона не выбрано назначение. Загрузка невозможна."; 
		Сообщение.Сообщить();
				
		//Сообщить("Для данного шаблона не выбрано назначение. Загрузка невозможна.", СтатусСообщения.Важное);
		Возврат Ложь;
	КонецЕсли; 
...
Функция СтрокуВПредставлениеТелефона(ЗНАЧ Строка) Экспорт
...
Функция НайтиСправочникПоРеквизитам(Данные, Реквизиты) Экспорт
...
Функция СоздатьЭлемент(Спр, Данные)
	Объект = Спр.СоздатьЭлемент();
	Для каждого СтрокаТЗ Из Данные Цикл
		Объект[СтрокаТЗ.Реквизит] = СтрокаТЗ.Значение;
	КонецЦикла;
	Объект.Записать();
	Возврат Объект.Ссылка;
КонецФункции

Функция ПустоеЗначение(ЗНАЧ Строка)
	
	Строка = СтрЗаменить(Строка, " ", "");
	Строка = СтрЗаменить(Строка, "-", "");
	
	Возврат ПустаяСтрока(Строка);
