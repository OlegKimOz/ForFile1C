...
Функция ЕстьЗаполненныеСвойстваКонтактнойИнформации(Знач Владелец)
	...
	Если Владелец.Свойство("Value") = Неопределено Или Владелец.Свойство("Type") = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ЭтоТипАдрес(Владелец.Type) Тогда
		...
		Для каждого ИмяПоля Из СписокПолейДляПроверки Цикл
			Если Владелец.Свойство(ИмяПоля) <> Неопределено И ЗначениеЗаполнено(Владелец[ИмяПоля]) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
