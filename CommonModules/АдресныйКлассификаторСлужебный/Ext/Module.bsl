...
Функция ПредставлениеДомаИСтроений(Знач НазваниеВладения, Знач Дом, Знач Корпус, Знач НазваниеСтроения, Знач Строение)
	...
	Если ЗначениеЗаполнено(Корпус) Тогда
		СписокСтроений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, НСтр("ru='корпус'"), Корпус));
	КонецЕсли;
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		...
		Иначе
			ТекстПроСтроения = Новый ФорматированнаяСтрока(НСтр("ru = 'корпус'") + " ",
				Новый ФорматированнаяСтрока(СтрокаПоиска, Новый Шрифт(,, Истина) , ЦветаСтиля.РезультатУспехЦвет),
...
Функция КодыДомовВладенийИСтроений()
	...
	Результат.Строения.Вставить(ВРег(НСтр("ru='Корпус'")), -1);
...
Функция КодыПомещений()
	...
	Результат = Новый Соответствие;
	Результат.Вставить(ВРег(НСтр("ru = 'Комната'")),   1);
	Результат.Вставить(ВРег(НСтр("ru = 'Помещение'")), 2);
	Результат.Вставить(ВРег(НСтр("ru = 'Квартира'")),  3);
	Результат.Вставить(ВРег(НСтр("ru = 'Офис'")),      4);
	Результат.Вставить(ВРег(НСтр("ru = 'Бокс'")),      5);
	Результат.Вставить(ВРег(НСтр("ru = 'Этаж'")),      6);
	Результат.Вставить(ВРег(НСтр("ru = 'А/я'")),       7);
	Результат.Вставить(ВРег(НСтр("ru = 'В/ч'")),       8);
	Результат.Вставить(ВРег(НСтр("ru = 'П/о'")),       9);
