...
Процедура Подключаемый_ЭлементНастройки_ПриИзменении(Элемент)
	...
	Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
		...
		ОбновитьФорму(ПараметрыОбновления);
	Иначе
		ОтчетыКлиент.ОбновитьСвойстваНастройки(ЭтотОбъект, КомпоновщикНастроек, ЭлементНастройки);
...
Функция ПредставлениеПолейГруппировки(ЭлементНастройки, ПометкаУдаления)
	...
	Для Каждого Элемент Из Поля.Элементы Цикл 
		...
		ПредставлениеПолей = ПредставлениеПолей
			+ ?(ЗначениеЗаполнено(ПредставлениеПолей), ", ", "")
			+ ?(ОписаниеПоля = Неопределено, Строка(Элемент.Поле), ОписаниеПоля.Заголовок);
		
		Если Элемент.ТипГруппировки <> ТипГруппировкиКомпоновкиДанных.Элементы Тогда 
			ПредставлениеПолей = ПредставлениеПолей + " (" + Элемент.ТипГруппировки + ")";
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьПризнакиВложенныхЭлементовНастроек(ЭлементСтруктуры, СвойстваЭлементаСтруктуры)
	...
	СвойстваЭлементаСтруктуры.СодержитОтборы = ЭлементСтруктуры.Отбор.Элементы.Количество();
	СвойстваЭлементаСтруктуры.СодержитУсловноеОформление = ЭлементСтруктуры.УсловноеОформление.Элементы.Количество();
	
	ВложенныеЭлементы = ЭлементСтруктуры.Выбор.Элементы;
	...
	ВложенныеЭлементы = ЭлементСтруктуры.Порядок.Элементы;
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	ОтображатьВыбранныеПоля =
		ЗначениеЗаполнено(СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки) Или ЭтоРасширенныйРежим;
